<template>
  <div id="app">
    <b-navbar class="navbar navbar-dark bg-primary" toggleable="lg" type="dark" variant="primary">
      ...
      <b-collapse id="nav-collapse" is-nav>
        <b-navbar-nav>
          ...
          <b-nav-item href="https://github.com/KozmaAdam/ErettsegiKeritesTsVueJs">GitHub</b-nav-item>
        </b-navbar-nav>
      </b-collapse>
    </b-navbar>
    <TxtReader label="Forrás (kerites.txt):" placeholder="Bedobó mező" @load="txtSorai = $event" />
    <div v-if="mutat" id="megoldas" class="p-3 mb-2 bg-primary text-white border border-dark rounded-lg">
      ...
      <p>
        ...
        <input v-model="hazszamInputStr" class="bg-primary text-white" type="number" min="1" max="117" /><br />
    ...
    <p v-if="mutat">
      <button class="btn btn-primary btn-outline-primary">
        <font-awesome-icon icon="clouduploadalt" class="mr-1" />
        <TxtWriter title="utcakep.txt állomány mentése" :content="utcakep" filename="utcakep.txt" />
      </button>
    </p>
    
    <div v-if="mutat" id="egyebek">
      <pre class="w-50 p-3 mb-2 bg-primary text-white border border-dark rounded-lg">
utcakep.txt fájl:
{{ utcakep }}
      </pre>
      <pre class="w-25 p-3 mb-2 bg-primary text-white border border-dark rounded-lg">
kerites.txt fájl:
{{ txtSorai }}
      </pre>
    </div>
